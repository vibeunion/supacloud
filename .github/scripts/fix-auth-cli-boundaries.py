...
source = sub_once(
    r'const AUTH_LINK_TYPES = \[.*?\] as const;',
...
source = sub_once(
    r'const SAFE_USER_FIELDS = \[.*?\] as const;\n',
...
source = sub_once(
    r'function safeRedirectTo\(candidate: unknown\): string \| undefined \{.*?\n\}\n\nfunction isRecord',
...
source = sub_once(
    r'function projectUser\(candidate: unknown\): Record<string, unknown> \| null \{.*?\n\}\n\nfunction safeAuthReadFailure',
...
source = sub_once(
    r'async function listUsers\(http: HttpTransport, args: Record<string, unknown>\) \{.*?\n\}\n\nasync function getUser',
...
source = sub_once(
    r'async function getUser\(http: HttpTransport, args: Record<string, unknown>\) \{.*?\n\}\n\nasync function generateLink',
...
source = sub_once(
    r'async function generateLink\(http: HttpTransport, args: Record<string, unknown>\) \{.*?\n\}\n\nfunction formatProviders',
...
marker = '    test("decodes a CLI JSON config before sending the PATCH request", async () => {'
if tests.count(marker) != 1:
    raise SystemExit("auth test insertion marker was not unique")
inserted = r'''    test("rejects malformed or cross-user auth projections without reflecting remote fields", async () => {
...
'''
test_path.write_text(tests.replace(marker, inserted + marker, 1), encoding="utf-8")
