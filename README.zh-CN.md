...
**SupaCloud** 是面向 AI 辅助开发的应用工程底座，同时提供基于 **Pigsty** 的 Supabase 风格项目多租户私有化运行平台。

### 工程目标

- **可靠基础层**：把重复的执行、持久化与治理机制收敛到经过测试的契约。
- **便捷 Vibe Coding**：提供标准项目模板、局部应用上下文、可操作的编译诊断与快速本地验证。
- **提前发现错误**：结合类型、静态编译与运行时 Schema；编译通过不等于业务正确。
- **大型项目可维护性**：通过明确的模块归属与静态声明的 AOP 管理横切行为。

应用框架（`@supacloud/app`、`@supacloud/compiler`、`@supacloud/elysia`）负责应用结构与执行契约，运行平台负责项目隔离、基础设施集成与交付。**SupAuth 是企业应用的外部统一用户中心依赖**，不在各应用内重复建设用户体系；业务关系和对象级授权仍由应用负责。

职责、已有基础、待补工作与验收标准见[工程目标](docs/engineering-goals.zh-CN.md)。
