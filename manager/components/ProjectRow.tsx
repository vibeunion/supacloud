...
export const ProjectRow = ({ name, runtime = "deno", lang = 'en' }: { name: string, runtime?: string, lang?: Lang }) => (
    <div className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-white/5 transition-colors group">
        ...
        <div className="col-span-3 flex items-center gap-3">
            ...
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${runtime === 'bun' ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' : 'bg-blue-500/10 text-blue-400 border-blue-500/20'}`}>
        ...
        <div className="col-span-2 text-right opacity-0 group-hover:opacity-100 transition-opacity flex justify-end gap-2">
            ...
            <button
                hx-post={`/projects/${name}/runtime`}
                hx-vals={runtime === 'bun' ? '{"runtime": "deno"}' : '{"runtime": "bun"}'}
                hx-swap="none"
                title="Switch Runtime"
                className="text-slate-400 hover:bg-white/10 hover:text-cyan-400 px-3 py-1.5 rounded text-xs font-medium transition-colors"
                onclick="setTimeout(() => window.location.reload(), 2000)"
            >
                {runtime === 'bun' ? '⚡️' : '🦕'}
            </button>
