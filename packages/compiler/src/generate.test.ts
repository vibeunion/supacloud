...
describe("generate：application.ts 关键内容", () => {
  test("generated typed providers, factories and runtime descriptors pass a consumer typecheck", async () => {
    const root = await mkdtemp(join(tmpdir(), "supacloud-typed-consumer-"));
    try {
      const graph: ApplicationGraph = {
        externalTokens: ["DB"],
        modules: [{
          name: "typed", className: "TypedModule", file: "source.ts", line: 1,
          ...
          providers: [
            ...
            {
              token: "LABEL", kind: "factory", tokenKind: "injection-token", useFactoryName: "makeLabel",
              importPath: "source", scope: "application", deps: ["DB"],
              exported: false, file: "source.ts", line: 1,
            },
          ],
          controllers: [{
            className: "HttpController", path: "/", scope: "request",
            deps: ["Service", "LABEL"], importPath: "source", file: "source.ts",
...
export class HttpController {
  constructor(readonly service: Service, readonly label: string) {}
  hello(): string { return this.label; }
}
`,
...
interface RuntimeModule {
  controllers: Array<{
    ...
    routes: Array<{ method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE" | "HEAD" | "OPTIONS" }>;
  }>;
}
...
`,
  ...
  test("property-level inject() is rejected for compile-time DI", () => {
