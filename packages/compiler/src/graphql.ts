...
export async function renderGraphql(options: CompileOptions): Promise<GraphqlArtifacts> {
  ...
  try {
    ...
    // Operations v6 owns used enums and inputs. Emitting the full TypeScript
    // plugin too duplicates those declarations; retain its scalar/helper API only.
    const baseTypes = new typescript.TsVisitor(schema, config);
    const prelude = [...baseTypes.getWrapperDefinitions(), baseTypes.scalarsDefinition].filter(Boolean).join("\n") + "\n";
    const generated = prelude + await codegen({
      filename: "graphql.ts",
