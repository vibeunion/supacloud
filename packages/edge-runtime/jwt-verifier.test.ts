...
describe("verifyEdgeRuntimeJwt", () => {
  ...
  test("preserves legal Latin-1 subjects without normalization", () => {
    for (const subject of ["café", "mañana", "ÿ", "user\u00a0name"]) {
      const trusted = withVerifiedJwtContext(
        new Request("https://example.com/functions/v1/fa"),
        subject,
      );

      expect(trusted.headers.get(VERIFIED_JWT_SUB_HEADER)).toBe(subject);
    }
  });
