...
const requestIdFromHeader: NonNullable<ApplicationOptions["requestContext"]> = (request) => ({
...
describe("createApplication", () => {
  ...
  test("compiled constructor wiring isolates concurrent async handlers without runtime DI", async () => {
    ...
    const module: CompiledModule = {
      ...
      createRequestScope: (services, context) => {
        if (!context || typeof context !== "object" || !("requestId" in context)
          ...
          throw new Error("Invalid explicit request dependencies");
        }
      ...
      controllers: [{
        ...
        routes: [{
          ...
          path: "/context",
          handler: "get",
          response: t.Object({ requestId: t.String(), rootValue: t.String() }),
        }],
      }],
    };
      ...
      const [first, second] = await Promise.all([
        testRequest(app, "/injected/context", { headers: { "x-request-id": "request-a" } }),
        testRequest(app, "/injected/context", { headers: { "x-request-id": "request-b" } }),
      ]);
