...
function normalizedFeatureKey(key: string): string {
  return key.trim().toLowerCase().replaceAll("-", "_");
}

function isUnsupportedAuthFeature(key: string): boolean {
  const normalized = normalizedFeatureKey(key);
  return normalized.startsWith("passkey")
    || normalized.startsWith("webauthn")
    || normalized.startsWith("mfa_web_authn")
    || normalized.startsWith("mfa_webauthn");
}

function isUnsupportedMfaFeature(key: string): boolean {
  const normalized = normalizedFeatureKey(key);
  return normalized.startsWith("webauthn") || normalized.startsWith("web_authn");
}

function scrubMfaWebAuthn(auth: Record<string, unknown>, projectRef: string): boolean {
  let changed = false;
  for (const [key, rawMfa] of Object.entries(auth)) {
    if (normalizedFeatureKey(key) !== "mfa") continue;
    const mfa = strictConfigObject(rawMfa, `projects.config.auth.${key}[${projectRef}]`);
    if (!mfa) continue;
    let mfaChanged = false;
    for (const mfaKey of Object.keys(mfa)) {
      if (!isUnsupportedMfaFeature(mfaKey)) continue;
      delete mfa[mfaKey];
      mfaChanged = true;
    }
    if (!mfaChanged) continue;
    auth[key] = mfa;
    changed = true;
  }
  return changed;
...
function scrubProjectWebAuthnConfig(rawConfig: unknown, projectRef: string): Record<string, unknown> | null {
  ...
  for (const key of Object.keys(auth)) {
    if (!isUnsupportedAuthFeature(key)) continue;
    delete auth[key];
    changed = true;
  }
  changed = scrubMfaWebAuthn(auth, projectRef) || changed;
  if (!changed) return null;
  config.auth = auth;
  return config;
}

export async function migrateUnsupportedWebAuthnConfig(db: SQL): Promise<number> {
  ...
  for (const project of projects) {
    const config = scrubProjectWebAuthnConfig(project.config, project.ref);
    if (!config) continue;
    await db`
      UPDATE projects
  ...
  if (migrated > 0) logger.info(`[PlatformV2] Removed unsupported WebAuthn config from ${migrated} project(s)`);
  return migrated;
}
