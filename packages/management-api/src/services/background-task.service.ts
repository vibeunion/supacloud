...
function captureEnqueueInput(value: unknown): EnqueueBackgroundFunctionTaskInput {
  ...
  if (Object.keys(captured).some((key) => !allowed.has(key))) throw new InvalidBackgroundInvocationError();
  const {
  ...
  if (typeof projectRef !== "string" || projectRef.length === 0
    ...
    || !isRecord(rawEnvelope)) throw new InvalidBackgroundInvocationError();
  const requiredEnvelopeKeys = [
    "method", "path", "query", "headers", "body", "body_encoding", "auth", "requested_timeout_sec",
  ];
  if (requiredEnvelopeKeys.some((key) => !Object.hasOwn(rawEnvelope, key))) throw new InvalidBackgroundInvocationError();
...
function taskReplayMatches(
  ...
  if (task.project_ref !== input.projectRef
    ...
    || task.auth_authority_ref !== authAuthorityRef) return false;
  try {
...
export async function enqueueBackgroundFunctionTask(
  ...
  return withRetry("BackgroundTaskService.enqueueBackgroundFunctionTask", () => sql.begin(async (transaction) => {
    const insertTask = () => transaction`
      INSERT INTO project_tasks (
        project_ref, task_type, function_slug, function_version, status, payload,
        max_attempts, next_run_at, timeout_sec, idempotency_key, trace_id,
        correlation_id, business_task_id, invoker_user_id, auth_authority_ref, metadata
      )
      VALUES (
        ${input.projectRef}, ${TaskType.EDGE_FUNCTION}, ${input.functionSlug},
        ${input.functionVersion || null}, ${TaskStatus.PENDING}, ${envelope},
        ${maxAttempts}, NOW(), ${timeoutSec}, ${idempotencyKey}, ${input.traceId},
        ${input.correlationId ?? null}, ${input.businessTaskId ?? null},
        ${invokerUserId}::uuid, ${authAuthorityRef}, ${input.metadata ?? null}
      )
      RETURNING *
    `;
    ...
    if (idempotencyKey) {
      const existingRows: unknown[] = await transaction`
        ...
        WHERE project_ref = ${input.projectRef} AND idempotency_key = ${idempotencyKey}
        FOR UPDATE
      `;
      ...
      if (existingRow !== undefined) {
        ...
        if (!taskReplayMatches(existingTask, input, envelope, timeoutSec, maxAttempts, authAuthorityRef, invokerUserId)) {
          throw new BackgroundTaskIdempotencyConflictError();
        }
        rows = await transaction`
          UPDATE project_tasks SET updated_at = NOW()
          WHERE id = ${existingTask.id} AND project_ref = ${input.projectRef}
    ...
    assertPersistedTask(task, input, envelope, timeoutSec, maxAttempts, authAuthorityRef, invokerUserId, isNewTask);
    return task;
  }), {
