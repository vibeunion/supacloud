...
export async function startPgflowTask(
    ...
    // Conflict checks occur in SQL, not after a repository upsert that may alter another task.
...
export async function reconcilePgflowTask(id: string, ref: string): Promise<void> {
    // Keep the platform row locked until the tenant run receipt is persisted. A crash between
    // databases is repaired using the same UUID, never by starting a second run.
    await sql.begin(async (tx) => {
        ...
        const run = await db.begin(async (tenant) => {
            ...
            if (task.status !== "pending") {
                throw new PgflowTaskError(503, "pgflow run is missing; refusing to replay");
            }
