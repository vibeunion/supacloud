...
export async function setupCiBridge(sql: InstanceType<typeof SQL>): Promise<{
  ...
  //    (these go into public schema, accessible to anon/authenticated roles)
  const dbUrl = config.databaseUrl;
  const testSql = new SQL({ url: dbUrl, max: 2 });
  try {
    // todos — the canonical SDK test table
    await testSql`
    ...
    // Grant full access to anon + authenticated roles for SDK testing
    // (We disable RLS — these are test tables, security is irrelevant in CI)
    await testSql`
    ...
    `;
    await testSql`
      GRANT USAGE, SELECT ON SEQUENCE public.todos_id_seq
        TO anon, authenticated, service_role
    `;
    await testSql`ALTER TABLE public.todos DISABLE ROW LEVEL SECURITY`;
    ...
    console.log(
      "[CIBridge] Test tables created and PostgREST schema cache refreshed.",
    );
  } finally {
    await testSql.close();
  }
