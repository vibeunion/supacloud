...
import { switchover, reloadConfig, addReplica } from "../../src/services/maintenance.service";
...
describe("MaintenanceService", () => {
    test("switchover should call ha_manager.sh with correct args", async () => {
        ...
        const result = await switchover("db-cluster", "node-2");
    ...
    test("reloadConfig should call ha_manager.sh reload", async () => {
        ...
        const result = await reloadConfig("192.168.1.10");
    ...
    test("addReplica should trigger async expansion", async () => {
        ...
        const result = await addReplica("192.168.1.11");
