...
test("SDK replays the real authenticated Management route and official Auth SDK over native HTTP", async () => {
  ...
  const upstream = Bun.serve({
    ...
    async fetch(request) {
      ...
      expect(request.headers.get("x-project-ref")).toBe("a");
      expect(request.headers.get("authorization")).not.toBe(`Bearer ${config.masterToken}`);
      if (request.method === "DELETE") { removed = true; return new Response(null, { status: 204 }); }
      if (path.endsWith("/regenerate_secret")) return Response.json({ ...saved, client_secret: "rotated" });
      if (request.method === "POST") return Response.json({ ...saved, client_secret: "created" }, { status: 201 });
      if (request.method === "PUT") {
        const body: unknown = await request.json();
        if (!body || typeof body !== "object" || !("client_name" in body) || typeof body.client_name !== "string") throw new Error("Invalid fixture input");
        saved = { ...saved, client_name: body.client_name };
      }
  ...
  const project = spyOn(projectAuthRepository, "findByRef").mockResolvedValue({
    ref: "a", organization_id: null, jwt_secret: "synthetic-signing-secret",
    config: {
  ...
  try {
    const api = new SupaCloudOAuthClientsClient({
      managementApiUrl: management.url.origin, projectRef: "a", getAccessToken: () => config.masterToken,
    });
    expect((await api.list()).clients).toHaveLength(1);
    expect((await api.get(id)).client_id).toBe(id);
    expect((await api.create({ redirect_uris: client().redirect_uris })).client_secret).toBe("created");
    expect((await api.update(id, { client_name: "Changed" })).client_name).toBe("Changed");
    expect((await api.regenerateSecret(id)).client_secret).toBe("rotated");
    await api.delete(id);
    expect(await api.list()).toEqual({ clients: [] });
    expect(upstreamPaths).toHaveLength(7);
    const unauthorized = new SupaCloudOAuthClientsClient({
      managementApiUrl: management.url.origin, projectRef: "a", getAccessToken: () => "invalid",
    });
    expect((await rejected(unauthorized.list())).status).toBe(401);
    expect(upstreamPaths).toHaveLength(7);
  } finally {
    project.mockRestore(); config.masterToken = oldToken; config.authRuntimeOwnerRef = oldOwner;
