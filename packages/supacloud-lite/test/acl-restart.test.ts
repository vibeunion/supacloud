...
test('restart preserves application table, sequence, schema and default privileges', async () => {
  ...
  try {
    ...
    const acl = async () => (await backend!.db.query(`
    ...
    `)).rows[0]
    expect(await acl()).toEqual({ table_select: false, sequence_usage: false, schema_usage: false })
    await backend.close()
    ...
    const defaults = await backend.db.query(`select
