...
test('current generated starter runs through Lite bundling, routes, contracts and command governance', async () => {
  ...
  // Runtime imports keep the compiler/CLI's separate TypeScript projects out of Lite's declaration build.
  const { initializeAppProject } = await import(new URL('../../cli/src/shared/tools/app-starter.ts', import.meta.url).href)
  ...
  try {
    ...
    await symlink(join(import.meta.dir, '../node_modules'), join(project, 'node_modules'), 'dir')
    ...
    expect(result.diagnostics.filter((item: { severity: string }) => item.severity === 'error')).toEqual([])
    for (const name of ['api', 'denied']) {
      ...
      await writeFile(join(project, `supabase/functions/${name}/index.ts`), `
        ...
        sandbox.app.get('/native', () => validatedJsonResponse(
          (value): value is { ok: boolean } => typeof value === 'object' && value !== null && typeof value.ok === 'boolean',
          { ok: true }
        ));
...
`)
    ...
    expect(await (await fetch(`${base}/api/reviews/health`)).json()).toEqual({ ok: true })
    expect(await (await fetch(`${base}/api/native`)).json()).toEqual({ ok: true })
    ...
    expect(await first.json()).toEqual({ state: 'approved', version: 2 })
    const replay = await approve({ expectedVersion: 1 }, 'once')
    expect(replay.status).toBe(200)
    expect(await replay.json()).toEqual({ state: 'approved', version: 2 })
