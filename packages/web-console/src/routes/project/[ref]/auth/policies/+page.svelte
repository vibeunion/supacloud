<script lang="ts">
  ...
  const POLICY_ACTION_KEYS: Record<string, string> = {
    ...
    UPDATE: "AuthPolicies.action_update",
    DELETE: "AuthPolicies.action_delete",
    ALL: "AuthPolicies.action_all",
  };
  ...
  const saveMutation = createMutation(() => ({
    ...
    onSuccess: async () => {
      ...
      toast.success($t("AuthPolicies.created"));
    },
    onError: (err: unknown) => {
      addError = (err instanceof Error ? err.message : String(err)) || $t("AuthPolicies.create_failed");
    }
  }));

  async function savePolicy() {
    if (!newPolName || !newPolTable) {
      addError = $t("AuthPolicies.name_and_table_required");
      return;
    }
    addError = null;
    saveMutation.mutate();
  ...
  const deleteMutation = createMutation(() => ({
    ...
    onError: (err: unknown) => {
      toast.error(`${$t("AuthPolicies.delete_failed")}: ${err instanceof Error ? err.message : String(err)}`);
    }
  }));

  async function deletePolicy(policy: RlsPolicy) {
    if (!confirm($t("AuthPolicies.delete_confirmation", { values: { name: policy.policyname } }))) return;
  ...
  function policyActionLabel(command: string): string {
    const translationKey = POLICY_ACTION_KEYS[command];
    return translationKey ? $t(translationKey) : command;
  }

  function policyTypeLabel(permissive: string): string {
    if (permissive === "PERMISSIVE") return $t("AuthPolicies.type_permissive");
    if (permissive === "RESTRICTIVE") return $t("AuthPolicies.type_restrictive");
    return permissive;
  }
</script>
...
<div class="h-full flex flex-col space-y-4">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-2xl font-bold">{$t("AuthPolicies.title")}</h1>
      <p class="text-sm text-muted-foreground mt-1">{$t("AuthPolicies.subtitle")}</p>
    </div>
    <div class="flex items-center gap-3">
      <span class="px-2.5 py-1 rounded-full bg-brand/10 text-brand text-xs font-bold">{$t("AuthPolicies.policy_count", { values: { count: policies.length } })}</span>
      <button 
      ...
      >
        <Plus size={14} /> {$t("AuthPolicies.create")}
      </button>
    </div>
  </div>
  ...
  {#if showAdd}
    <div class="rounded-xl border bg-card p-5 space-y-4">
      <h3 class="text-sm font-semibold">{$t("AuthPolicies.new_policy")}</h3>
      {#if addError}
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <span class="text-xs text-muted-foreground">{$t("AuthPolicies.policy_name")}</span>
          <input type="text" bind:value={newPolName} placeholder={$t("AuthPolicies.policy_name_placeholder")}
            class="w-full mt-1 px-3 py-2 text-xs rounded-lg border bg-background focus:outline-none focus:ring-1 focus:ring-brand" />
        </div>
        <div>
          <span class="text-xs text-muted-foreground">{$t("AuthPolicies.target_table")}</span>
          <select bind:value={newPolTable} class="w-full mt-1 px-3 py-2 text-xs rounded-lg border bg-background focus:outline-none focus:ring-1 focus:ring-brand">
            <option value="" disabled>{$t("AuthPolicies.select_table")}</option>
            {#each tables as tbl}
        ...
        <div>
          <span class="text-xs text-muted-foreground">{$t("AuthPolicies.action")}</span>
        ...
        <div>
          <span class="text-xs text-muted-foreground">{$t("AuthPolicies.target_roles")}</span>
          <input type="text" bind:value={newPolRoles} placeholder={$t("AuthPolicies.roles_placeholder")}
            class="w-full mt-1 px-3 py-2 text-xs rounded-lg border bg-background focus:outline-none focus:ring-1 focus:ring-brand" />
        </div>
        
        <div class="md:col-span-2">
          <span class="text-xs text-muted-foreground">{$t("AuthPolicies.using_expression")}</span>
          <textarea bind:value={newPolUsing} placeholder={$t("AuthPolicies.using_placeholder")} rows="2"
        ...
        <div class="md:col-span-2">
          <span class="text-xs text-muted-foreground">{$t("AuthPolicies.with_check_expression")}</span>
          <textarea bind:value={newPolWithCheck} placeholder={$t("AuthPolicies.with_check_placeholder")} rows="2"
            class="w-full mt-1 px-3 py-2 text-xs font-mono rounded-lg border bg-background focus:outline-none focus:ring-1 focus:ring-brand"></textarea>
        </div>
        {/if}
      </div>
      ...
      <div class="flex items-center justify-end gap-3 pt-2">
        <button onclick={() => showAdd = false} class="px-4 py-2 text-xs font-medium rounded-lg hover:bg-muted/50 transition-colors">{$t("Common.cancel")}</button>
  ...
  <div class="relative max-w-sm">
    ...
    <input
      ...
      placeholder={$t("AuthPolicies.search_placeholder")}
  ...
  <div class="flex-1 rounded-xl border border-border/50 bg-background shadow-sm overflow-hidden">
    {#if isLoading}
      <div class="flex flex-col items-center justify-center py-24 text-muted-foreground gap-3">
        ...
        <p class="text-xs font-mono uppercase tracking-widest">{$t("AuthPolicies.loading")}</p>
      </div>
    {:else if policies.length === 0}
      <div class="flex flex-col items-center justify-center py-24 text-muted-foreground gap-3">
        <Shield size={40} class="opacity-20" />
        <p class="text-sm">{$t("AuthPolicies.empty")}</p>
        <p class="text-xs">{$t("AuthPolicies.empty_description")}</p>
      </div>
    {:else}
      <div class="overflow-auto max-h-[65vh]">
        <table class="w-full text-left text-xs">
          <thead class="bg-muted/30 border-b sticky top-0">
            <tr>
              <th class="px-4 py-2.5 font-semibold text-muted-foreground">{$t("AuthPolicies.policy_name")}</th>
              <th class="px-4 py-2.5 font-semibold text-muted-foreground">{$t("AuthPolicies.table")}</th>
              <th class="px-4 py-2.5 font-semibold text-muted-foreground">{$t("AuthPolicies.action")}</th>
              <th class="px-4 py-2.5 font-semibold text-muted-foreground">{$t("AuthPolicies.type")}</th>
              <th class="px-4 py-2.5 font-semibold text-muted-foreground">{$t("AuthPolicies.roles")}</th>
              <th class="px-4 py-2.5 font-semibold text-muted-foreground">{$t("AuthPolicies.condition")}</th>
            </tr>
          </thead>
            ...
            {#each filteredPolicies as policy}
              <tr class="hover:bg-muted/5 transition-colors">
                ...
                <td class="px-4 py-2.5">
                  <span title={policy.cmd} class="px-1.5 py-0.5 rounded text-[9px] font-bold {getCmdColor(policy.cmd)}">{policyActionLabel(policy.cmd)}</span>
                </td>
                <td class="px-4 py-2.5">
                  <span title={policy.permissive} class="px-1.5 py-0.5 rounded text-[9px] {policy.permissive === 'PERMISSIVE' ? 'text-green-600 bg-green-500/10' : 'text-red-600 bg-red-500/10'}">{policyTypeLabel(policy.permissive)}</span>
                </td>
