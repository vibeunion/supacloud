...
<div class="mx-auto max-w-4xl space-y-5 pb-10">
  ...
  {:else if upgrade}
    <section class="space-y-4 rounded-xl border bg-card p-5">
      <div class="flex flex-wrap items-center justify-between gap-3"><div><h2 class="font-semibold">当前工作流</h2><p class="mt-1 text-xs text-muted-foreground">状态：<span class="font-mono">{upgrade.status || upgrade.upgrade_status}</span> · 当前版本 {upgrade.current_major || upgrade.current_version || "未知"}</p></div>{#if upgrade.status === "manual_recovery_required"}<button onclick={rollback} disabled={saving} class="rounded-md border border-destructive/40 px-3 py-1.5 text-xs text-destructive">请求回滚</button>{/if}</div>
