...
function compiler(): BunPlugin {
  ...
  return {
    name: "hosting-environment-tests",
    setup(builder) {
      builder.onResolve({ filter: /^\$app\/state$/ }, () => ({ path: localFile("env.test-fixture.svelte.ts") }));
      builder.onResolve({ filter: /^\$lib\// },
...
test("compiled hosting forms bind configuration snapshots, environment revisions, and A-B-A completion", async () => {
  if (process.env.SUPACLOUD_HOSTING_ENV_TEST !== "1") {
    const child = Bun.spawn({
      cmd: [process.execPath, "test", fileURLToPath(import.meta.url)],
      env: { ...process.env, SUPACLOUD_HOSTING_ENV_TEST: "1" }, stdout: "pipe", stderr: "pipe",
    });
  ...
  const bundlePath = join(tmpdir(), `supacloud-hosting-env-${crypto.randomUUID()}.mjs`);
  try {
    const result = await Bun.build({
      entrypoints: [localFile("env.test-entry.ts")], target: "browser", format: "esm",
      conditions: ["svelte", "browser"], external: ["node:assert"], plugins: [compiler()],
    });
    if (!result.success) throw new AggregateError(result.logs, "Hosting environment fixture build failed");
    const output = result.outputs[0];
    if (!output) throw new Error("Missing hosting environment bundle");
