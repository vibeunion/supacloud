...
function compiler(): BunPlugin {
  ...
  return {
    name: "hosting-list-tests",
    setup(builder) {
      builder.onResolve({ filter: /^(\$app\/(state|navigation)|svelte-i18n)$/ },
        () => ({ path: localFile("list.test-fixture.svelte.ts") }));
      builder.onResolve({ filter: /^\$lib\/(api|hosting-list|hosting-mutations)$/ },
      ...
      builder.onLoad({ filter: /\.svelte\.[jt]s$/ }, async ({ path }) => {
        ...
        return {
          contents: compileModule(path.endsWith(".ts") ? typescript.transformSync(source) : source,
...
test("compiled hosting list validates data and isolates reads and writes through A-B-A navigation", async () => {
  if (process.env.SUPACLOUD_HOSTING_LIST_TEST !== "1") {
    const child = Bun.spawn({
      cmd: [process.execPath, "test", fileURLToPath(import.meta.url)],
      env: { ...process.env, SUPACLOUD_HOSTING_LIST_TEST: "1" }, stdout: "pipe", stderr: "pipe",
    });
  ...
  const bundlePath = join(tmpdir(), `supacloud-hosting-list-${crypto.randomUUID()}.mjs`);
  try {
    const result = await Bun.build({
      entrypoints: [localFile("list.test-entry.ts")], target: "browser", format: "esm",
      conditions: ["svelte", "browser"], external: ["node:assert"], plugins: [compiler()],
    });
    if (!result.success) throw new AggregateError(result.logs, "Hosting list fixture build failed");
    const output = result.outputs[0];
    if (!output) throw new Error("Missing hosting fixture bundle");
