<script lang="ts">
  ...
  const generalMutation = createMutation(() => ({
    mutationFn: async () => {
      ...
      if (!res.ok) {
        let message = $t("ProjectSettings.general_save_failed");
        try {
          const err = await res.json();
          message = err?.message || err?.error || message;
        } catch {}
    ...
    onError: (err: unknown) => {
      actionMsg = `❌ ${err instanceof Error ? err.message : $t("ProjectSettings.general_save_failed")}`;
    },
    onSettled: () => {
      actionInProgress = null;
      setTimeout(() => actionMsg = null, 4000);
  ...
  {:else if project}
    <div class="space-y-6">
      ...
      <div class="border rounded-xl bg-card p-6 space-y-4">
        ...
        <form
          class="space-y-3"
          onsubmit={(event) => { event.preventDefault(); saveGeneralSettings(); }}
        >
          ...
          <button
            type="submit"
            class="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-brand text-white text-sm font-medium disabled:opacity-50"
            disabled={actionInProgress !== null || !generalName.trim()}
          >
            {#if actionInProgress === "general"}
