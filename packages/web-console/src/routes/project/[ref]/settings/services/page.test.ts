...
function compiler(): BunPlugin {
  ...
  return {
    name: "service-control-tests",
    setup(builder) {
      builder.onResolve({ filter: /^(\$app\/(state|paths)|svelte-sonner)$/ },
        () => ({ path: localFile("page.test-fixture.svelte.ts") }));
      builder.onResolve({ filter: /^\$lib\/(api|project-services)$/ },
...
test("compiled service controls reject invalid state and isolate ownership, project and pending operations", async () => {
  if (process.env.SUPACLOUD_SERVICE_CONTROL_TEST !== "1") {
    const child = Bun.spawn({
      cmd: [process.execPath, "test", fileURLToPath(import.meta.url)],
      env: { ...process.env, SUPACLOUD_SERVICE_CONTROL_TEST: "1" }, stdout: "pipe", stderr: "pipe",
    });
  ...
  const bundlePath = join(tmpdir(), `supacloud-service-control-${crypto.randomUUID()}.mjs`);
  try {
    ...
    if (!result.success) throw new AggregateError(result.logs, "Service control fixture build failed");
    const output = result.outputs[0];
    if (!output) throw new Error("Missing service control fixture bundle");
