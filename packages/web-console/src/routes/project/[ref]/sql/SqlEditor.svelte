<script lang="ts">
  ...
  interface QueryTab {
    ...
    error: string | null;
    explainResults: Record<string, unknown>[] | null;
    command: string | null;
  ...
  let { projectRef }: { projectRef: string } = $props();
  let disposed = false;
  ...
  onMount(() => {
    ...
    try {
      ...
      try {
        ...
        if (oldDraft) initial.sql = oldDraft;
    ...
    function handleClickOutside(e: MouseEvent) {
      ...
      if (!(target instanceof Element) || !target.closest(".role-menu-container")) {
        showRoleMenu = false;
  ...
  function saveTabs(key: string, snapshot: string) {
    ...
    saveTimeout = setTimeout(() => {
      try {
        ...
        saveFailed = true;
        toast.error($t("SqlEditor.save_failed"));
      } finally {
        isSaving = false;
      }
    }, 1000);
  ...
  const runQueryMutation = createMutation(() => ({
    retry: false,
    mutationFn: async ({ rawSql, role, projectRef, explainMode, queryId }: SqlMutationVariables) => {
      let queryToRun = rawSql;
      if (explainMode) {
        queryToRun = `EXPLAIN (ANALYZE, COSTS, VERBOSE, FORMAT TEXT) ${rawSql}`;
      }
      const wrappedSql = wrapSqlWithRole(queryToRun, role);
    ...
    onSuccess: (result, variables) => {
      ...
      tabs = tabs.map(tb => {
        ...
        if (variables.explainMode) {
          return { ...tb, explainResults: recordRows(result.rows), results: null, error: null, command: result.command, statementCount: result.statementCount, rowCount: result.rowCount, durationMs: result.durationMs };
        } else {
          return { ...tb, results: recordRows(result.rows), explainResults: null, error: null, command: result.command, statementCount: result.statementCount, rowCount: result.rowCount, durationMs: result.durationMs };
        }
      });
    },
  ...
  async function cancelQuery() {
    ...
    try {
      const response = await apiClient(
        `/v1/projects/${runningQuery.projectRef}/database/sql/${encodeURIComponent(runningQuery.queryId)}/cancel`,
        { method: "POST" },
      );
      await readDatabaseSqlCancellationResponse(response);
      if (!disposed && activeQuery?.queryId === runningQuery.queryId) toast.success($t("SqlEditor.cancel_confirmed"));
    } catch (error) {
      if (!disposed && activeQuery?.queryId === runningQuery.queryId) {
        toast.error(error instanceof Error ? error.message : $t("SqlEditor.cancel_failed"));
      }
    } finally {
      if (!disposed && activeQuery?.queryId === runningQuery.queryId) isCancelling = false;
    }
  }
...
<div class="h-[calc(100vh-12rem)] flex flex-col space-y-3">
  <div class="flex flex-wrap items-center justify-between gap-3">
    ...
    <div class="flex flex-wrap items-center gap-3">
      ...
      {:else if saveFailed}
        <span role="alert" class="text-[10px] text-destructive">{$t("SqlEditor.save_failed")}</span>
      {:else}
  ...
  <div class="flex-1 flex flex-col min-h-0 rounded-xl border border-border/50 bg-background shadow-sm overflow-hidden">
    <!-- Editor Header -->
    <div class="flex flex-wrap items-center px-4 py-2 border-b bg-muted/30 gap-4">
      ...
      <div class="relative role-menu-container min-w-0 max-w-full">
        <button
          onclick={() => { showRoleMenu = !showRoleMenu; }}
          class="flex flex-wrap items-center gap-2 px-3 py-1 rounded-md text-xs font-medium transition-all border max-w-full break-all {isImpersonating ? 'bg-amber-500/10 border-amber-500/30 text-amber-600' : 'bg-muted/50 border-transparent text-muted-foreground hover:bg-muted'}"
        >
      ...
      <div class="flex-1 overflow-auto p-4">
            ...
            <table class="w-full text-left text-[11px] font-mono border-collapse min-w-max">
              <thead class="bg-muted/60 dark:bg-muted/30 border-b border-border sticky top-0 z-10">
                <tr>
                  ...
                  {#if activeTab.results.length > 0}
                    {#each Object.keys(activeTab.results[0] ?? {}) as key (key)}
              ...
              <tbody class="divide-y divide-border/30 dark:divide-border/20">
                {#each activeTab.results as row, idx (`${idx}-${JSON.stringify(row)}`)}
                  <tr class="hover:bg-muted/30 dark:hover:bg-muted/10 transition-colors group">
                    ...
                    {#each Object.keys(activeTab.results[0] ?? {}) as key, valueIndex (`${idx}-${valueIndex}`)}
                      {@const value = row[key]}
        ...
        {:else if activeTab?.explainResults}
          ...
          <div class="space-y-0 font-mono text-xs">
            {#each activeTab.explainResults as row, idx (`${idx}-${String(row["QUERY PLAN"] || "")}`)}
