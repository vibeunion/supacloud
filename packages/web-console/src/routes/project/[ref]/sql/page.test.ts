...
function compiler(): BunPlugin {
  ...
  return {
    name: "sql-editor-tests",
    setup(builder) {
      builder.onResolve({ filter: /^\$app\/state$|^\$lib\/api$|^svelte-sonner$/ }, () => ({
        path: localFile("page.test-fixture.svelte.ts"),
      }));
      builder.onResolve({ filter: /^\$lib\// }, ({ path }) => ({
        path: localFile(`../../../../lib/${path.slice(5)}.ts`),
      }));
...
test("SQL editor isolates projects, validates responses and survives unavailable storage", async () => {
  if (process.env.SUPACLOUD_SQL_EDITOR_TEST !== "1") {
    const child = Bun.spawn({
      cmd: [process.execPath, "test", fileURLToPath(import.meta.url)],
      env: { ...process.env, SUPACLOUD_SQL_EDITOR_TEST: "1" },
  ...
  const bundlePath = join(tmpdir(), `supacloud-sql-editor-${crypto.randomUUID()}.mjs`);
  try {
    const result = await Bun.build({
      entrypoints: [localFile("page.test-entry.ts")],
      target: "browser", format: "esm", conditions: ["svelte", "browser"], external: ["node:assert"],
      plugins: [compiler()],
    });
    if (!result.success) throw new AggregateError(result.logs, "SQL fixture build failed");
    const output = result.outputs[0];
    if (!output) throw new Error("Missing SQL fixture bundle");
