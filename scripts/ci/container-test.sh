...
# 关键：在容器内必须禁用 hosts/dns 修改（由于挂载限制）和内核模块加载（由于权限限制）
export SUPACLOUD_ANSIBLE_ARGS="-e node_write_etc_hosts=false -e node_dns_method=none -e node_kernel_modules=[] -vv"
