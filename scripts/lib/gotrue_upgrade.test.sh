...
good_archive="${tmp_dir}/auth-v2.193.0.tar.xz"
wrong_version_archive="${tmp_dir}/auth-v2.194.0.tar.xz"
make_gotrue_archive v2.193.0 "$good_archive"
...
curl() {
    ...
    if [[ "${FORCE_NEW_HEALTH_MISMATCH:-false}" == true && "$installed" == v2.193.0 ]]; then
...
export GOTRUE_VERSION=v2.193.0
